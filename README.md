...
可以使用 `Ctrl+Shift+U`（macOS 为 `Command+Shift+U`）直接翻译当前页面，或使用 `Ctrl+Shift+Y`（macOS 为 `Command+Shift+Y`）打开本译。侧边栏会显示 Chrome 实际分配的键位，点击“修改快捷键”即可进入配置页面；也可以手动打开 `chrome://extensions/shortcuts`。
