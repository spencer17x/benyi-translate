...
- 语言检测实例在当前页面任务生命周期内复用。
- 整页翻译实例在当前页面任务生命周期内复用；划词翻译实例在当前页面生命周期内复用。
...
- `storage`：保存译文颜色等非敏感界面偏好，不保存正文、网址或译文。

当前版本只使用 `storage` 保存界面偏好；站点规则或受限持久化缓存实际实现后，必须另行补充对应存储用途和隐私验收。
