...
│          ├──────────────┬──────────────┐              │
│          ▼              ▼              ▼              │
│  Side Panel       Offscreen Host   Content Script     │
│  可选控制与状态    模型、队列、分块   发现与安全渲染    │
│                         │              ▲              │
│                         └──────────────┘              │
│                         │                             │
│                         ▼                             │
...
- 直接触发当前页面翻译，或按需打开 Side Panel。
- 创建和复用不显示 UI 的 Offscreen Document 翻译宿主。
...
- 用户关闭 Side Panel 后任务继续运行；重新打开后从 Content Script 恢复当前状态和进度。

### 2.4 Offscreen Translation Host

- 使用扩展自带的隐藏文档承载仅支持 Document 上下文的 Translator 和 Language Detector API。
- 不显示窗口、标签页或侧边栏，只能通过 `chrome.runtime` 接收已校验的任务消息。
- 每个任务内部顺序翻译，使用 `tabId + pageId + taskId` 隔离并丢弃迟到结果。

### 2.5 Translation Engine
...
- 每个页面任务维护独立逻辑队列，单个任务内严格顺序翻译。
- 当前视口任务使用最高优先级，临近视口次之，屏外任务最低。
- 跨页面任务由 Chrome 本地语言运行时协调资源，不由扩展绕过浏览器限制强制并发。
...
- 语言检测实例在翻译宿主生命周期内复用。
- 整页翻译实例在 Offscreen Host 的任务生命周期内复用；划词翻译实例在当前页面生命周期内复用。
- 任务完成、取消、页面导航或宿主关闭时销毁对应实例。
...
- `offscreen`：运行无法在 Service Worker 中使用的 Chrome 本地语言 API，不显示额外 UI。
- `sidePanel`：提供可选的控制与状态界面。

当前版本不申请 `storage` 权限；站点规则或受限持久化缓存实际实现后再增加对应存储用途。
...
- 页面刷新、标签关闭和扩展重新加载。
- Side Panel 关闭后继续翻译、重新打开恢复状态、标签切换和多标签页任务身份校验。
